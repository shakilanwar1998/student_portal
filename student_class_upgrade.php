            ...
            if(isset($_REQUEST["class"]))
            {
              if($_REQUEST["class"]!=0)
              {
                $class = $_REQUEST["class"];
                    ...
                    $select_query = "SELECT * FROM resultsheet INNER JOIN student_auth on student_auth.Student_ID = resultsheet.student_id
                                    INNER JOIN student on student_auth.Student_ID = student.student_id WHERE student.class = $class AND resultsheet.class = $class ";

                    $run_select_query = mysqli_query($connect,$select_query);

                    while($data = mysqli_fetch_array($run_select_query))
                    {
